    ...
    .get(verifyProfessorOwnsCourse, async (req, res) => {
        try {
            ...
            return res.render("professorDashboard/viewOfficeHours", {
                ...
                taOfficeHours,
                events: events,
                calendarSubscribeLink: subscribeLink
            });
        } catch (e) {
            return res.status(500).render("error", {
                error:
    ...
    .post(verifyProfessorOwnsCourse, async (req, res) => {
        try {
            ...
            if (!ObjectId.isValid(studentId) || !ObjectId.isValid(courseId)) {
                throw "Invalid student or course ID.";
            }

            const usersCol = await users();
            const lecturesCol = await lectures();
            const coursesCol = await courses();

            const student = await usersCol.findOne({_id: new ObjectId(studentId)});
            ...
            await usersCol.updateOne({_id: new ObjectId(studentId)}, updates);

            // 📚 Sync existing lectures to TA's calendar
            try {
                const courseDoc = await coursesCol.findOne({_id: new ObjectId(courseId)});
                const lectures = await lecturesCol
                    .find({courseId: new ObjectId(courseId)})
                    .toArray();

                for (const lecture of lectures) {
                    if (!lecture.lectureDate || !lecture.lectureStartTime || !lecture.lectureEndTime) {
                        console.warn(`⛔ Skipping invalid lecture: ${lecture.lectureTitle || lecture._id}`);
                        continue;
                    }

                    lecture.courseCode = courseDoc.courseCode;

                    // Use existing calendar sync method — this will insert to new TA calendar as well
                    const newEventIds = await addLectureToCalendars(lecture);

                    // Merge new TA calendar event ID into lecture document
                    const taEventId = newEventIds?.tas?.[studentId];
                    if (taEventId) {
                        await lecturesCol.updateOne(
                            {_id: lecture._id},
                            {$set: {[`calendarEventIds.tas.${studentId}`]: taEventId}}
                        );
                    }
                }
            } catch (e) {
                console.error("⚠️ Error syncing lectures to TA calendar:", e.message);
            }

            // 🕐 Sync professor office hours to TA calendar
            try {
                const courseDoc = await coursesCol.findOne({_id: new ObjectId(courseId)});
                const taUser = await usersCol.findOne({_id: new ObjectId(studentId)});

                if (!taUser?.calendarId) {
                    console.warn("⚠️ TA has no calendarId, skipping office hour sync.");
                } else if (Array.isArray(courseDoc.professorOfficeHours)) {
                    const name = `${taUser.firstName} ${taUser.lastName}`;

                    for (const oh of courseDoc.professorOfficeHours) {
                        if (!oh.day || !oh.startTime || !oh.endTime || !oh.location) {
                            console.warn("⛔ Skipping invalid office hour.");
                            continue;
                        }
                        ...
                        const result = await addOfficeHourEvent({
                            ...
                            calendarId: taUser.calendarId,
                        });

                        if (result?.eventId) {
                            // Store the new TA event ID into the office hour record
                            await coursesCol.updateOne(
                                {
                                    _id: new ObjectId(courseId),
                                    "professorOfficeHours.day": oh.day,
                                    "professorOfficeHours.startTime": oh.startTime,
                                    "professorOfficeHours.endTime": oh.endTime,
                                },
                                {
                                    $set: {
                                        [`professorOfficeHours.$.calendarEventIds.tas.${studentId}`]: result.eventId,
                                    },
                                }
                            );
                        } else {
                            console.warn(`⚠️ Failed to sync OH for day ${oh.day} to TA ${taUser.email}`);
                        }
                    }
                }
            } catch (e) {
                console.error("⚠️ Error syncing office hours to TA calendar:", e.message);
            }
