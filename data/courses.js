...
const addProfessorOfficeHour = async (courseId, officeHourObj) => {
    ...
    if (professor.calendarId) {
        ...
        if (result?.eventId) {
            calendarEventIds.professors[professor._id.toString()] = result.eventId;
        }
    }

    // Add to TA calendars
    for (const taId of currentCourse.taIds || []) {
        const ta = await usersCollection.findOne({_id: new ObjectId(taId)});
        if (ta?.calendarId) {
            const result = await addOfficeHourEvent({
                name,
                day,
            ...
            if (result?.eventId) {
                calendarEventIds.tas[ta._id.toString()] = result.eventId;
            }
        }
    }

    // Add to student calendars
    for (const enrollment of currentCourse.studentEnrollments || []) {
        if (enrollment.status !== "active") continue;
        const student = await usersCollection.findOne({_id: new ObjectId(enrollment.studentId)});
        if (student?.calendarId) {
            ...
            if (result?.eventId) {
                calendarEventIds.students[student._id.toString()] = result.eventId;
            }
        }
    }
...
const deleteProfessorOfficeHour = async (courseId, officeHourObj) => {
    ...
    const officeHour = course.professorOfficeHours.find(
        (oh) =>
            oh.day === day && oh.startTime === startTime && oh.endTime === endTime
    );
    ...
    if (officeHour.calendarEventIds) {
        const professor = await usersCollection.findOne({_id: course.professorId});
        if (professor?.calendarId && officeHour.calendarEventIds.professors?.[professor._id.toString()]) {
            const eventId = officeHour.calendarEventIds.professors[professor._id.toString()];
            await deleteOfficeHourEvent(professor.calendarId, eventId);
        }


        for (const taId of course.taIds || []) {
            const ta = await usersCollection.findOne({_id: new ObjectId(taId)});
            const eventId = officeHour.calendarEventIds?.tas?.[taId.toString()];
            if (ta?.calendarId && eventId) {
                await deleteOfficeHourEvent(ta.calendarId, eventId);
            }
        }

        for (const {studentId, status} of course.studentEnrollments || []) {
            if (status !== "active") continue;
            const student = await usersCollection.findOne({_id: new ObjectId(studentId)});
            const eventId = officeHour.calendarEventIds?.students?.[studentId.toString()];
            if (student?.calendarId && eventId) {
                await deleteOfficeHourEvent(student.calendarId, eventId);
            }
        }
    }
